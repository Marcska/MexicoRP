...
AddEventHandler("playerConnecting", function(name, setReason, deferrals)
	SinglePlayerLogs('**' ..GetPlayerName(source).. '** se está conectando al servidor.	', Config.BaseColors['log-in'], source, 'log-in')
end)

-- Send message when Player disconnects from the server
AddEventHandler('playerDropped', function(reason)
	SinglePlayerLogs('**' ..GetPlayerName(source).. '** se está desconectando. (Reason: ' .. reason .. ')', Config.BaseColors['log-out'], source, 'log-out')
end)

-- Send message when Player creates a chat message (Does not show commands)
AddEventHandler('chatMessage', function(source, name, msg)
	SinglePlayerLogs('**' .. sanitize(GetPlayerName(source)) .. '** escribió: `' .. msg..'`', Config.BaseColors['chat'], source, 'chat')
end)
...
AddEventHandler('playerDied',function(id,player,killer,DeathReason, Weapon)
	...
	if id == 1 then  -- Suicide/died
        SinglePlayerLogs('**' .. sanitize(GetPlayerName(source)) .. '** se suicidó con '.._Weapon, Config.BaseColors['deaths'], source, 'deaths') -- sending to deaths channel
	elseif id == 2 then -- Killed by other player
		DualPlayerLogs('**' .. GetPlayerName(killer) .. '** fue asesinado por ' .. GetPlayerName(source).. ' `('.._Weapon..')`', Config.BaseColors['deaths'], killer, source, 'deaths') -- sending to deaths channel
	else -- When gets killed by something else
        SinglePlayerLogs('**' .. GetPlayerName(source) .. '** Se murió ', Config.BaseColors['deaths'], source, 'deaths') -- sending to deaths channel
	end
end)
...
AddEventHandler('playerShotWeapon', function(weapon)
	...
	if Config.weaponLog then
		SinglePlayerLogs('**' .. GetPlayerName(source)  .. '** disparó la siguiente arma `' .. weapon .. '`', Config.BaseColors['shooting'], source, 'shooting')
    end
end)
...
AddEventHandler('onResourceStop', function (resourceName)
    HidePlayerDetails('** se ha detenido el siguiente recurso ' .. resourceName .. '**', Config.BaseColors['resources'], 'resources')
end)

-- Send message when a resource is being started
AddEventHandler('onResourceStart', function (resourceName)
    Wait(100)
    HidePlayerDetails('** se inició el siguiente recurso ' .. resourceName .. '**', Config.BaseColors['resources'], 'resources')
end)
